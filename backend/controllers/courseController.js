...
// FIXED: Enrollment check now works for all authenticated users
exports.getCourseDetails = async (req, res) => {
  try {
    ...
    // FIXED: Check enrollment for ANY authenticated user, not just students
    let isEnrolled = false;
    if (req.userId) { // Check if user is authenticated (has userId)
      try {
        isEnrolled = await Enrollment.isEnrolled(req.userId, courseId);
        console.log(`✅ Enrollment check: user ${req.userId} in course ${courseId} = ${isEnrolled}`);
      } catch (enrollError) {
        console.warn('Could not check enrollment status:', enrollError.message);
      }
    } else {
      console.log('ℹ️  No user ID - user not authenticated');
    }
