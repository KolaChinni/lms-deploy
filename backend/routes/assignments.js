...
const submissionValidation = [
  ...
  body().custom((value, { req }) => {
    const hasText = req.body.submission_text && req.body.submission_text.trim().length > 0;
    const hasFile = req.file;
    
    if (!hasText && !hasFile) {
      throw new Error('Either submission text or file upload is required');
    }
...
router.get('/student/courses/:courseId/assignments', 
  auth, 
  assignmentController.getStudentCourseAssignments
);

// FIXED: Submission routes with updated validation
