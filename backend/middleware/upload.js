...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    if (file.fieldname === 'video') {
      ...
      // FIX: Added 'file' fieldname for assignment submissions
...
const fileFilter = (req, file, cb) => {
  if (file.fieldname === 'video') {
    ...
    // FIX: Added 'file' fieldname for assignment submissions
    // Allow document files for both 'document' and 'file' fieldnames
    const allowedTypes = [
      'application/pdf',
      'application/msword',
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      cb(new Error('Only PDF, DOC, PPT, TXT, and ZIP files are allowed for document upload'), false);
    }
  } else {
